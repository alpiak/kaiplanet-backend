...
export default class KuaidailiProducer extends Producer implements IProducer {
    public static get instances() {
        return KuaidailiProducer.config.producers.kuaidaili.instances
    ...
    constructor(host?: string, port?: number, protocol = "https", options?: IProducerConstructOptions) {
        super(host, port, protocol, {
            ...options,
            userAgent: KuaidailiProducer.config.producers.kuaidaili.userAgent,
        });
    }
    ...
    public async fetchProxyList(length: number, area = Area.GLOBAL, { abortSignal }: IProducerOptions = {}) {
        ...
        return entries.map((entry: string[]) => {
            ...
            if (entry[3].toLowerCase() === "https") {
                const httpsProxy = new Proxy(entry[0], "https", parseInt(entry[1], 10), area);
